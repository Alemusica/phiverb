...
def _format_bounds(models):
    sab = models["sabine"]
    eyr = models["eyring"]
    lo_raw, hi_raw = models["raw_bounds"]
    lo_guard, hi_guard = models["guard_bounds"]
    return (
        f"Sabine={sab:.3f}s Eyring={eyr:.3f}s "
        f"raw=[{lo_raw:.3f},{hi_raw:.3f}] "
        f"guard=[{lo_guard:.3f},{hi_guard:.3f}] "
        f"(±{int(models['buffer']*100)}% guard)"
    )
...
def validate_ir(wav, fs_expect, volume, surface, alpha_bar, m_air, tol):
    ...
    if not in_bounds:
        ...
        raise AssertionError(
            f"T={T:.3f}s fuori dal range [{lo_guard:.3f},{hi_guard:.3f}] "
            f"({_format_bounds(models)})"
        )
    if not in_tol:
        raise AssertionError(
            f"T={T:.3f}s oltre ±{int(tol*100)}% dal modello best={models['best']:.3f}s "
            f"({_format_bounds(models)})"
        )
...
def main():
    ...
    for scene_json in sorted(scenes_path.glob("*.json")):
        ...
        try:
            ...
            print(
                f"[QA] {scene_json.name}: T20={T20:.3f}s T30={T30:.3f}s "
                f"{_format_bounds(models)} best={models['best']:.3f}s"
            )
